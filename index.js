...
// ANSWER: function findFourth(numbers) {if (numbers.length >= 4) {return numbers [3]}else {return null}}
...
/* function getNumbersSquare(numbers) {
  ...
  for (let i = 0; i < numbers.length; i++) {
    const squaredValue = numbers[i] * numbers[i]
    answers.push(squaredValue)
  }
  return answers
} 
...
// ANSWER: No they do not, countLetters is an O(1) while countLettersRoundTwo is an O(k)


// Q: What is the space complexity of each? 
// ANSWER - countLetters: O(1)
// ANSWER - countLettersRoundTwo: O(k)


// Q: Explain how you came to your conclusions about the space complexity of each function. 
// ANSWER: After doing some research about O(k), I found that spae complexity depends on the number of unique letters in the input string.
/*

  ANSWER: After doing some research about O(k), I found that space complexity depends on the 
  number of unique letters in the input string.
  While O(1) has constant space complexity.
